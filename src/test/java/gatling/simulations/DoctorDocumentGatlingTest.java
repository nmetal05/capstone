...
public class DoctorDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all doctorDocuments").get("/api/doctor-documents").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new doctorDocument")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_doctorDocument_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created doctorDocument").get("${new_doctorDocument_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created doctorDocument").delete("${new_doctorDocument_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DoctorDocument entity").exec(scn);
