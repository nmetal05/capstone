 ...
 */
public class DoctorProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all doctorProfiles").get("/api/doctor-profiles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new doctorProfile")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"officeAddress\": \"SAMPLE_TEXT\"" +
                                ", \"latitude\": 0" +
                                ", \"longitude\": 0" +
                                ", \"inpeCode\": \"SAMPLE_TEXT\"" +
                                ", \"isVerified\": null" +
                                ", \"lastLoginIp\": \"SAMPLE_TEXT\"" +
                                ", \"lastUserAgent\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_doctorProfile_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created doctorProfile").get("${new_doctorProfile_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created doctorProfile").delete("${new_doctorProfile_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DoctorProfile entity").exec(scn);
