 ...
 */
public class AppUserProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all appUserProfiles").get("/api/app-user-profiles").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appUserProfile")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_appUserProfile_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created appUserProfile").get("${new_appUserProfile_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created appUserProfile").delete("${new_appUserProfile_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppUserProfile entity").exec(scn);
