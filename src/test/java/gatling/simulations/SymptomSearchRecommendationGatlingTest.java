...
/**
 * Performance test for the SymptomSearchRecommendation entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.10.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class SymptomSearchRecommendationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all symptomSearchRecommendations")
                    .get("/api/symptom-search-recommendations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new symptomSearchRecommendation")
                        .post("/api/symptom-search-recommendations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"confidenceScore\": 0" + ", \"rank\": 0" + ", \"reasoning\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_symptomSearchRecommendation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created symptomSearchRecommendation")
                            .get("${new_symptomSearchRecommendation_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created symptomSearchRecommendation")
                        .delete("${new_symptomSearchRecommendation_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SymptomSearchRecommendation entity").exec(scn);
