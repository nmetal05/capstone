...
public class DoctorViewHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all doctorViewHistories")
                    .get("/api/doctor-view-histories")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new doctorViewHistory")
                        .post("/api/doctor-view-histories")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"viewDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_doctorViewHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created doctorViewHistory").get("${new_doctorViewHistory_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created doctorViewHistory").delete("${new_doctorViewHistory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DoctorViewHistory entity").exec(scn);
